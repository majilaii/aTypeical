 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if(activeWordIndex > 0) {
        socket.emit("indexIncrease" , {textLength: gameState.words.length, activeWordIndex,socketID: socket.id, gameID: gameState._id})    
    }
    ...
    <div className="SocketContainer">
      ...
      <div className="typing-container socket-typing">
        {text.length > 0
          ? text.map((element, index) => {
              return (
                <Word element={element} activeWordIndex={activeWordIndex} key={index} listID={index} raceInputRef={raceInputRef} wrong = {wrong}/>
              );
            })
          : null}
      </div>
      ...
      <div className="inputDiv">
        ...
        ref={raceInputRef}
          type="text"
          className="inputBar"
          disabled = {gameStart===false || gameOver === true ? "disabled" : null}
          onChange={(e) => {
